                ...
                new List<Villa>()
                {
                    new Villa()
                    {
                        ...
                        Rooms = new List<Room>()
                        {
                            new Room()
                            {
                                Accommodations = new List<Accommodation>() { new Accommodation() { Сapacity = 2 }  }
                            },
                            new Room()
                            {
                                Accommodations = new List<Accommodation>()
                                {
                                    new Accommodation() { Сapacity = 2 },
                                    new Accommodation() { Сapacity = 3 }
                                }
                            },
                            new Room()
                            {
                                Accommodations = new List<Accommodation>() { new Accommodation() { Сapacity = 1 }  }
                            }
                        }
                    }
                }
        ...
        public async Task Assure_All_Orders_Calculate_Correctly()
        {
            ...
            bookingRepository.Setup(p => p.GetBookingsAsync(It.IsAny<DateTime?>())).ReturnsAsync(
                new List<Order>()
                {
                    ...
                    CreateTypicalOrder(DateTime.Now.AddDays(20), null, 2, OrderStatus.Request), // не должен учитываться в загрузке,
                    CreateTypicalOrder(DateTime.Now.AddDays(30), null, 2, OrderStatus.Request) // должен учитываться в загрузке
                }
            );
